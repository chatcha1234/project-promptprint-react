// ===== นำเข้า React และ Components =====
import React from "react";
import ProductCard from "../components/ProductCard";

// ===== Component หลัก: หน้าแรก (Home) =====
const Home = () => {
  // ===== ข้อมูลสินค้าตัวอย่าง (Mock Data) =====
  const products = [
    {
      id: 1,
    ...
    <div className="w-full">
      <div>
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold text-gray-900">Please select</h2>
        </div>

        {/* ===== Product Grid ===== */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {products.length > 0
            ? // ===== กรณีมีสินค้า: วนลูปแสดง ProductCard =====
              products.map((product) => (
                <ProductCard key={product.id} product={product} />
              ))
            : // ===== กรณีไม่มีสินค้า: แสดง Skeleton Loading =====
              [...Array(6)].map((_, index) => (
                <div
                  key={`skeleton-${index}`}
